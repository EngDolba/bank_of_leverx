...
namespace BankOfLeverx.Controllers
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        {
            var customer = await _customerRepository.GetByIdAsync(customerKey);
            if (customer is null)
            {
        ...
        {
            var updated = await _customerRepository.PatchAsync(customerKey, customerPatch);
            if (updated is null)
            {
                return NotFound($"Customer with Key {customerKey} not found.");
            }
        ...
        {
            var updated = await _customerRepository.UpdateAsync(customerKey, customer);
            if (updated is null)
            {
                return NotFound($"Customer with Key {customerKey} not found.");
            }
        ...
        {
            var deleted = await _customerRepository.DeleteAsync(customerKey);
            if (!deleted)
            {
                return NotFound($"Customer with key: {customerKey} not found");
            }
